...
#   2. Regenerate the spoti.pw remote-config flag table from the vanilla IPA
#      (Tools/SpotiPW/extract-flags.py — needs otool/nm/strings, all stock on macOS).
#   3. theos `make package FINALPACKAGE=1` — produces .deb with
#      EeveeSpotify.dylib + EeveeSpotify.bundle + framework.
#   4. Build zxPluginsInject.dylib — sideload compat shim (keychain redirect,
#      group containers, CloudKit stub). LC-injected via ipapatch in step 7.
...
#
# Requires: theos, cyan (pyzule-rw), ipapatch, dpkg, ldid, plutil, python3.
...
color "2/7  spoti.pw flag table (from the vanilla IPA)"
python3 Tools/SpotiPW/extract-flags.py "$VANILLA_IPA"

color "3/7  theos make package"
...
color "7/7  ipapatch LC-inject zxPluginsInject"
